...
def check(word, guesses):
    ...
    n3 = word.count(last_guess)
    print("That appears {} times.".format(n3))
    print("so far you've guessed", guesses)
...
def main():
    ...
    while not guessed:
        guess = input("Guess a {}-letter word: ".format(n))
        guess = guess.upper()
        n2 = len(guess)
        if guess == word:
            guessed = True
